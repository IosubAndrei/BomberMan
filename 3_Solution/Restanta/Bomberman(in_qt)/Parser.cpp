...
std::vector<std::string> Parser::unparseString(std::string parsedString, char stringDelimiter)
{
    ...
    istringstream check1(parsedString);
...
std::string Parser::hash_string(std::string sir, int nr_ordine, char delim)
{
    std::vector<std::string>vect;
    vect.push_back(sir);

    protocoliseString(nr_ordine, vect);

    std::string hased = parseString(vect, delim);
