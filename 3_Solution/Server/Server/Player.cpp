...
Player::Player(int ID, sf::Vector2f startingPosition) :
	m_maxBombCount(5), m_currentBombCount(1), m_bombsPlaced(0), m_currentBombExplosionSize(1),m_ID(ID), m_previousPosition(),
	m_position(startingPosition), m_newPosition(startingPosition),
	m_facingDirection(), m_movementFactor(0.0f), m_movementSpeed(2.5f), m_bombPlacementTimer(2.0f, eTimer::eTrue)
{}
...
void Player::update(float deltaTime)
{
	...
	{
		m_bombsPlaced = 0;
		m_bombPlacementTimer.resetElapsedTime();
