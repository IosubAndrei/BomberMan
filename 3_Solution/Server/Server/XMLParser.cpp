...
bool XMLParser::parseTextureDetails(sf::Vector2i& tileSize, sf::Vector2i& textureSize, int& column, const std::string& levelFileName, const std::string& textureFileName)
{
	TiXmlDocument file;
	bool fileLoaded = file.LoadFile(levelFileName.c_str());
	assert(fileLoaded);
		...
		tileSheetElement != nullptr; tileSheetElement = tileSheetElement->NextSiblingElement())
	{
		...
		if (tileSheetElement->Attribute("name") == textureFileName)
		{
			...
			column = textureSize.x / tileSize.x;
	...
	std::vector<sf::Vector2f>& spawnPosition)
{
	...
	if (!xmlFile.LoadFile(levelName.c_str()))
	{
		return false;
	}
	...
	spawnPosition = parseObjectLayer(*rootElement, tileSize, "Spawn Position Layer");
...
std::vector<std::vector<int>> XMLParser::decodeTileLayer(const TiXmlElement& tileLayerElement, sf::Vector2i mapSize)
{
	std::vector<std::vector<int>> tileData;
	tileData.reserve(mapSize.y);

	std::string decodeIDs;
	const TiXmlElement* dataNode = nullptr;
	...
	decodeIDs = base64.base64_decode(t);
	...
	for (int rows = 0; rows < mapSize.y; ++rows)
	{
		...
		{
			tileData[rows][cols] = *((int*)decodeIDs.data() + rows * mapSize.x + cols) - 1;
		}
	}
...
std::vector<TileLayer> XMLParser::parseTileLayers(const TiXmlElement& rootElement, const sf::Vector2i mapSize)
{
...
sf::Vector2i XMLParser::parseMapSize(const TiXmlElement& rootElement)
{
...
sf::Vector2i XMLParser::parseTileSize(const TiXmlElement& rootElement)
{
...
std::vector<sf::Vector2f> XMLParser::parseObjectLayer(const TiXmlElement& rootElement, sf::Vector2i tileSize, const std::string& layerName)
{
...
void XMLParser::parseCollisionLayer(const TiXmlElement& rootElement, sf::Vector2i tileSize, std::vector<std::vector<eCollidableTile>>& collisionLayer)
{
	std::vector<sf::Vector2f> objects;
	for (const auto* entityElementRoot = rootElement.FirstChildElement(); entityElementRoot != nullptr; entityElementRoot = entityElementRoot->NextSiblingElement())
	{
...
void XMLParser::parseBoxLayer(const TiXmlElement& rootElement, sf::Vector2i tileSize, std::vector<std::vector<eCollidableTile>>& collisionLayer)
{
	std::vector<sf::Vector2f> objects;
	for (const auto* entityElementRoot = rootElement.FirstChildElement(); entityElementRoot != nullptr; entityElementRoot = entityElementRoot->NextSiblingElement())
	{
