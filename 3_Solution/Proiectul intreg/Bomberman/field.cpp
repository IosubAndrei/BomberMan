...
Field::Field(int x, int y, QObject *parent) : QObject(parent), _x(x), _y(y), _isPlayerOn(false) {
    setPos(x, y);
    setPixmap(QPixmap(":/playing ground images/img/field.png"));
...
void Field::setUnDestroyableBlock(UnDestroyableBlock *unDestroyableBlock) {
    createPixmapItem(unDestroyableBlock, ":/playing ground images/img/unDestroyableBlock.png");
    _unDestroyableBlock = unDestroyableBlock;
}

void Field::setDestoryableBlock(DestroyableBlock *destroyableBlock) {
    createPixmapItem(destroyableBlock, ":/playing ground images/img/destroyableBlock.png");
    _destroyableBlock = destroyableBlock;
}

void Field::setBomb(Bomb *bomb) {
    createPixmapItem(bomb, ":/playing ground images/img/bomb.png");
    _bomb = bomb;
}
...
void Field::createExplosion() {
    ...
    createPixmapItem(_explosion, ":/playing ground images/img/fire.png");
    if(_isPlayerOn == true) {
        for (int i = 0; i < sizes::Players; i++) {
            if(_players[i])
